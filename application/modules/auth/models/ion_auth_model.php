	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$userid = $user->id;
				$usertypearray = $this->db->query("SELECT group_id FROM users_groups WHERE user_id = $userid")->row();
				$usertype = $usertypearray->group_id;
				$usergroup = array('user_type'=>$usertype);
				$this->session->set_userdata($usergroup);
